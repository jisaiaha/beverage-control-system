...
// Used for returning the WiFi ocnfiguration
...
// Event bits are used to indicate if an event has occured or not
...
// Queue handle used to manupulate the main queue of events
 ...
 * @param arg data, aside from the event data, that is passed to the handler when called
...
static void wifi_app_soft_ap_config(void){
    ...
    esp_netif_dhcps_stop(esp_netif_ap);                                 // Must call this before making DHCP changes
    ...
    ESP_ERROR_CHECK(esp_netif_dhcps_start(esp_netif_ap));               // Start the AP DHCP server (for connecting stations e.g. a mobile device)
    
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_APSTA));                     // Set the mode as AP / Station mode
...
/*
 * Connects the ESP32 to an external access point using the updated station configuration
 */
...
/*
 * Main task for the WiFi application, where we initialize WiFi
 * and recieve all queue messages to determine the flow of the application
            ...
            switch(msg.msgID){
                case WIFI_APP_MSG_LOAD_SAVED_CREDENTIALS:
                    ...
                    if(app_nvs_load_sta_creds()){
                        ESP_LOGI(TAG, "Loaded station configureation");
                ...
                case WIFI_APP_MSG_STA_CONNECTED_GOT_IP:
                    ...
                    if(eventBits & WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT){      // Save STA creds only if connecting from the http server (not loaded from NVS)
                        xEventGroupClearBits(wifi_app_event_group, WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT); // Clear the bits, in case we want to disconnect and reconnect, and then start again
                    }else{
                ...
                case WIFI_APP_MSG_STA_DISCONNECTED:
                    ...
                    if(eventBits & WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT){
                        ...
                        xEventGroupClearBits(wifi_app_event_group, WIFI_APP_CONNECTING_FROM_HTTP_SERVER_BIT);
                        http_server_monitor_send_message(HTTP_MSG_WIFI_CONNECT_FAIL);   // Only send if connecting from web server
                    }
...
/*
 * Pass the message to the function and then send it to the queue specifying the handle
 */
