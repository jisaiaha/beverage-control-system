...
// Queue handle to manipulte the main queue of events
static QueueHandle_t http_server_monitor_queue_handle;

/*
 * ESP32 timer configuration which is passed to esp_timer_create
 */
...
// Embedded files: jQuery, index.html, app.css, app.js and favicon.ico
...
/*
 * Checks the global firmware update status and creates the firmware update reset timer
...
static void http_server_fw_update_reset_timer(void){
    if(g_fw_update_status == OTA_UPDATE_SUCCESSFUL){
        ...
        // Give the web page a chance to recieve and acknoledge back and initialize a timer
 ...
 * @param req HTTP request for whic the uri needs to be handled
 * @return ESP_OK
 */
...
static esp_err_t http_server_favicon_ico_handler(httpd_req_t *req){
    ...
    httpd_resp_set_type(req, "image/image/x-icon");
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req){
    ...
    do{
        ...
        // Is this the first datat we are recieving
        ...
        if(!is_req_body_started){
            ...
            if(err != ESP_OK){
                ...
                printf("http_server_OTA_update_handler: Writing to partition subtype %d at offset 0x%ld\r\n", update_partition -> subtype, update_partition -> address);
            }
    ...
    if(esp_ota_end(ota_handle) == ESP_OK){
        ...
        if(esp_ota_set_boot_partition(update_partition) == ESP_OK){
            ...
            ESP_LOGI(TAG, "http_server_OTA_update_handler: Next boot partition subtype %d at offset 0x%ld", boot_partition -> subtype, boot_partition -> address);
...
static httpd_handle_t http_server_configure(void){
    ...
    xTaskCreatePinnedToCore(&http_server_monitor, "http_server_monitor", HTTP_SERVER_MONITOR_STACK_SIZE, NULL, HTTP_SERVER_MONITOR_PRIORITY, &task_http_server_monitor, HTTP_SERVER_TASK_CORE_ID);
    ...
    if(httpd_start(&http_server_handle, &config) == ESP_OK){
        ...
        httpd_register_uri_handler(http_server_handle, &http_server_disconnect_json_handler);
